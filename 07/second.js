...
function determineType(cards) {
  const cardMap = cards.split("").reduce(
    (acc, cur) => {
      acc[cur] ??= 0;
      acc[cur]++;
      return acc;
  ...
  const jokers = cardMap["J"];
  delete cardMap["J"];

  let highest = Object.entries(cardMap).reduce(
    (acc, cur) => (cur[1] > acc[1] ? cur : acc),
    [0, 0],
  );

  cardMap[highest[0]] += jokers;

  const numberOfEachCards = Object.values(cardMap);
